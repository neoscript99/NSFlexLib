package ns.flex.controls
{
	...
	public class DataGridPlus extends DataGrid
	{
		...
		[Bindable]
		public var showItemProxy:ObjectProxy;
		 ...
		 * @param evt
		 */
		protected function showItemDetail(item:Object, editable:Boolean=false):void
		{
			showItem=item;
			...
			{
				if (popEditing)
					popEditing.show(this);
				else
				{
					popEditing=initializePop(editable);
			...
			else
			{
				if (popView)
					...
					popView=initializePop(editable);
			}
		}

		private function initializePop(editable:Boolean=false):PopWindow
		{
